...
const HeartIcon: React.FC<{ filled?: boolean }> = memo(({ filled = false }) => (
    <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3" fill={filled ? "currentColor" : "none"} viewBox="0 0 24 24" stroke="currentColor">
...
const ArticleItemComponent: React.FC<ArticleItemProps> = ({ article, index }) => {
        ...
        <article className="h-full flex flex-col">
            ...
            <div className="flex flex-col h-full group">
                ...
                <div className="relative mb-4">
                    <LazyImage
                        src={article.imageUrl || `https://picsum.photos/seed/${article.link}/400/200`}
                        alt={`Thumbnail image for article: ${article.title}`}
                        className="w-full h-32 lg:h-40 object-cover rounded-lg"
                        srcSet={`${article.imageUrl || `https://picsum.photos/seed/${article.link}/200/100`} 200w,
                                ${article.imageUrl || `https://picsum.photos/seed/${article.link}/400/200`} 400w,
                                ${article.imageUrl || `https://picsum.photos/seed/${article.link}/600/300`} 600w`}
                    ...
                    >
                        ...
                        <div className="mt-auto space-y-2">
                            <div className="flex items-center justify-between text-xs text-gray-400">
                                ...
                                <span
                                    className="flex items-center gap-1 hover:text-white flex-shrink-0"
                                    aria-label={`${Math.floor(Math.random() * 100)} comments`}
                                    role="img"
                                >
                                    <ChatBubbleIcon />
...
const arePropsEqual = (prevProps: ArticleItemProps, nextProps: ArticleItemProps): boolean => {
    // Compare index
