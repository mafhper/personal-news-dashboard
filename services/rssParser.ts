...
function cleanDescription(description: string): string {
  // Use secure sanitization to prevent XSS and injection attacks
...
/**
 * Parse RSS XML content directly with security validations
 */
function parseRssXml(
  xmlContent: string,
  feedUrl: string
): { title: string; articles: Article[] } {
  // Use secure XML parsing to prevent CVE-2022-39353 and CVE-2021-21366
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    try {
      ...
      let descElement =
        descElements[0] || summaryElements[0] || contentElements[0];
      if (descElement?.textContent) {
        description = cleanDescription(descElement.textContent).substring(
          0,
          200
        );
      }
...
async function fetchRssWithCorsProxies(
  ...
  for (let i = 0; i < CORS_PROXIES.length; i++) {
    ...
    try {
      ...
      logger.debug(
        `Trying CORS proxy ${i + 1}/${CORS_PROXIES.length}: ${proxyUrl}`,
        {
          component: "rssParser",
      ...
      if (!response.ok) {
        throw new Error(
          `Proxy ${i + 1} failed: ${response.statusText} (${response.status})`
        );
      }
      ...
      if (proxyUrl.includes("allorigins.win")) {
        try {
          ...
          throw new Error(
            `Failed to parse allorigins response: ${
...
async function parseRssUrlWithRetry(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      perfDebugger.log(
        `Fetching RSS feed directly (attempt ${attempt}): ${url}`
      );

      // Fetch RSS content using CORS proxies with fallback
      const xmlContent = await fetchRssWithCorsProxies(url, {
        timeout,
        signal,
      });
      ...
      logger.warn(
        `Feed fetch attempt ${attempt} failed, retrying in ${delay}ms`,
        {
          component: "rssParser",
          additionalData: {
            feedUrl: url,
...
export async function parseRssUrl(
  ...
  if (!skipCache) {
    ...
    if (cachedArticles && cachedArticles.length > 0) {
      ...
      perfDebugger.log(
        `Using ${cachedArticles.length} smart cached articles for feed: ${url}`
      );
      return {
